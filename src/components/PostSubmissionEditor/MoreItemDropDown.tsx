...
const MoreItemDropDown: FC<Props> = ({ data = [], editor }) => {
  return (
    <Menu
      as="div"
      className="relative hidden lg:inline-block text-start ms-auto"
    >
      <Menu.Button className={"ms-auto menu-item"} title={"more"}>
      ...
      >
        <Menu.Items
          className={`absolute w-auto top-0 end-0 origin-top-right rtl:origin-top-left bg-white dark:bg-neutral-900 rounded-2xl divide-y divide-neutral-100 shadow-lg ring-1 ring-black dark:ring-white ring-opacity-5 dark:ring-opacity-10 focus:outline-none z-40`}
        >
          <div className="p-2.5 text-sm text-neutral-600 dark:text-neutral-300 flex items-center">
            {data.map((item, index) => (
